...
router.delete("/:id", (req, res) => {
  ...
  // Hard delete: remove habit completely from database
  db.prepare("DELETE FROM weekly_habits WHERE habit_id = ?").run(habitId);
...
router.put("/:id/toggle/:date", (req, res) => {
  ...
  if (existing) {
    ...
    db.prepare(
      "INSERT INTO habit_completions (habit_id, date, value) VALUES (?, ?, 1)",
    ).run(id, date);
...
router.put("/:id/value/:date", (req, res) => {
  ...
  if (existing) {
    db.prepare(
      "UPDATE habit_completions SET value = ? WHERE habit_id = ? AND date = ?",
    ).run(value, id, date);
  } else {
    db.prepare(
      "INSERT INTO habit_completions (habit_id, date, value) VALUES (?, ?, ?)",
    ).run(id, date, value);
  }
...
router.post("/copy-week", (req, res) => {
  ...
  if (!Array.isArray(habitIds) || !weekStart) {
    return res
      .status(400)
      .json({ error: "habitIds array and weekStart required" });
  }
