...
const HabitRow: React.FC<HabitRowProps> = ({
  ...
  const currentWeekActive =
    isCurrentWeek ??
    (days.some((day) => formatDate(day) === today) && days.length > 0);

  return (
    <div className="habit-row">
      <div className="habit-row-info">
        <span className="habit-emoji" style={{ color: habit.color }}>
          {habit.emoji}
        </span>
        <div
              ...
              {habit.type === "number" ? (
                ...
                isFuture || !currentWeekActive ? (
                  ...
                  <input
                    ...
                    value={
                      habit.completions[dateStr] !== undefined
                        ? (habit.completions[dateStr] as number)
                        : ""
                    }
                    placeholder="0"
                ...
                <button
                  className={`habit-check ${completed ? "completed" : ""} ${isToday ? "today" : ""} ${isFuture ? "future" : ""}`}
                  style={
                    completed
