...
interface AppContextValue {
  ...
  loading: boolean;
  error: string | null;
  addHabit: (habit: Omit<Habit, 'id' | 'createdAt' | 'completions' | 'activeWeeks'>, weekStart: string) => Promise<void>;
  deleteHabit: (id: string, weekStart?: string) => Promise<void>;
  toggleHabitDay: (habitId: string, dateStr: string) => Promise<void>;
  updateHabitValue: (habitId: string, dateStr: string, value: number) => Promise<void>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!habitsRes.ok || !goalsRes.ok) throw new Error('Server error loading data');
        const [habitsData, goalsData] = await Promise.all([habitsRes.json(), goalsRes.json()]);
  ...
  const addHabit = useCallback(async (habitData: Omit<Habit, 'id' | 'createdAt' | 'completions' | 'activeWeeks'>, weekStart: string) => {
    const newHabit = { ...habitData, id: uuidv4(), createdAt: new Date().toISOString(), weekStart };
  ...
  const deleteHabit = useCallback(async (id: string, weekStart?: string) => {
    try {
      const url = weekStart ? `${API}/habits/${id}?weekStart=${weekStart}` : `${API}/habits/${id}`;
  ...
  const toggleHabitDay = useCallback(async (habitId: string, dateStr: string) => {
    ...
    try {
      await fetch(`${API}/habits/${habitId}/toggle/${dateStr}`, { method: 'PUT' });
    } catch {
      // Revert on failure
      setHabits(prev => prev.map(h =>
        h.id === habitId
  ...
  const updateHabitValue = useCallback(async (habitId: string, dateStr: string, value: number) => {
    // Optimistic update
    setHabits(prev => prev.map(h =>
      h.id === habitId
  ...
  const copyWeekHabits = useCallback(async (habitIds: string[], weekStart: string): Promise<number> => {
  ...
  return (
    <AppContext.Provider value={{
      habits, goals, loading, error,
      addHabit, deleteHabit, toggleHabitDay, updateHabitValue, copyWeekHabits,
